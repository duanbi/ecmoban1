...
function favourable_goods_list($user_rank, $favourable_id, $sort = "", $order = "", $size, $page, $warehouse_id = 0, $area_id = 0)
{
	...
	$user_rank = "," . $user_rank . ",";
	$now = gmtime();
	$sql = "SELECT act_range_ext, act_range, userFav_type, user_id FROM " . $GLOBALS["ecs"]->table("favourable_activity") . " WHERE CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%' AND start_time <= '$now' AND end_time >= '$now' AND act_id = '$favourable_id' ";
...
function get_act_type($user_rank, $favourable_id)
{
	$user_rank = "," . $user_rank . ",";
	$now = gmtime();
	$sql = "SELECT act_type FROM " . $GLOBALS["ecs"]->table("favourable_activity") . " WHERE CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%' AND start_time <= '$now' AND end_time >= '$now' AND act_id = '$favourable_id' ";
