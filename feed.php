...
if (isset($_REQUEST["type"])) {
	if ($_REQUEST["type"] == "group_buy") {
	...
	else if ($_REQUEST["type"] == "auction") {
		$now = gmtime();
		$sql = "SELECT act_id, act_name, act_desc, start_time FROM " . $GLOBALS["ecs"]->table("goods_activity") . "WHERE act_type = '" . GAT_AUCTION . "' AND start_time <= '$now' AND is_finished < 3 ORDER BY start_time DESC";
		$res = $db->query($sql);
	...
	else if ($_REQUEST["type"] == "exchange") {
		$sql = "SELECT g.goods_id, g.goods_name, g.goods_brief, g.last_update FROM " . $GLOBALS["ecs"]->table("exchange_goods") . " AS eg, " . $GLOBALS["ecs"]->table("goods") . " AS g WHERE eg.goods_id = g.goods_id";
		$res = $db->query($sql);
		...
		$now = gmtime();
		$sql = "SELECT act_id, act_name, start_time FROM " . $GLOBALS["ecs"]->table("favourable_activity") . " WHERE start_time <= '$now' AND end_time >= '$now'";
		$res = $db->query($sql);
		...
		$now = gmtime();
		$sql = "SELECT act_id, act_name, act_desc, start_time FROM " . $GLOBALS["ecs"]->table("goods_activity") . "WHERE act_type = '" . GAT_PACKAGE . "' AND start_time <= '$now' AND is_finished < 3 ORDER BY start_time DESC";
		$res = $db->query($sql);
	...
	else if (substr($_REQUEST["type"], 0, 11) == "article_cat") {
		$sql = "SELECT article_id, title, author, add_time FROM " . $GLOBALS["ecs"]->table("article") . " WHERE is_open = 1 AND " . get_article_children(substr($_REQUEST["type"], 11)) . " ORDER BY add_time DESC LIMIT " . $_CFG["article_page_size"];
		$res = $db->query($sql);
	...
	$in_cat = (0 < $cat ? " AND " . get_children($cat) : "");
	$sql = "SELECT c.cat_name, g.goods_id, g.goods_name, g.goods_brief, g.last_update FROM " . $ecs->table("category") . " AS c, " . $ecs->table("goods") . " AS g WHERE c.cat_id = g.cat_id AND g.is_delete = 0 AND g.is_alone_sale = 1 " . $brd . $cat . "ORDER BY g.last_update DESC";
	$res = $db->query($sql);
