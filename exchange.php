...
{
	return $GLOBALS["db"]->getRow("SELECT keywords, cat_desc, style, grade, filter_attr, parent_id FROM " . $GLOBALS["ecs"]->table("category") . " WHERE cat_id = '$cat_id'");
}
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
	...
	$sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, eg.exchange_integral, g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot " . $select . "FROM " . $GLOBALS["ecs"]->table("exchange_goods") . " AS eg, " . $GLOBALS["ecs"]->table("goods") . " AS g WHERE eg.goods_id = g.goods_id AND $where $ext ORDER BY $sort $order";
