...
function get_myprice($id)
{
	...
	if (!empty($_SESSION["user_id"])) {
		...
		for ($count = count($my_price_time); $i < $count; $i++) {
			$bid_price[] = array("price" => price_format($my_price_time[$i]["bid_price"], false), "user_name" => $user_name, "bid_date" => local_date("Y-m-d H:i:s", $my_price_time[$i]["bid_time"]), "is_only" => in_array($my_price_time[$i]["bid_price"], $my_only_price));
		}
	...
	$sql = "SELECT end_time FROM " . $GLOBALS["ecs"]->table("goods_activity") . " WHERE act_id = '$id' AND act_type=" . GAT_SNATCH;
...
function get_snatch_list($keywords = "", $size, $page, $sort, $order, $warehouse_id, $area_id)
{
	...
	$now = gmtime();
	$sql = "SELECT ga.act_id AS snatch_id, ga.act_name AS snatch_name, ga.end_time, ga.start_time, ga.ext_info, IFNULL(g.goods_thumb, '') AS goods_thumb, g.market_price, IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '{$_SESSION["discount"]}') AS shop_price, IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price, g.promote_start_date, g.promote_end_date FROM " . $GLOBALS["ecs"]->table("goods_activity") . " AS ga LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON ga.goods_id = g.goods_id LEFT JOIN " . $GLOBALS["ecs"]->table("member_price") . " AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '{$_SESSION["user_rank"]}' " . $leftJoin . " WHERE ga.start_time <= '$now' AND ga.act_type=" . GAT_SNATCH . $where . "  ORDER BY $sort $order";
	$snatch_list = array();
	$overtime = 0;
...
function get_snatch($id)
{
	...
	if ($goods) {
		...
		$goods["store_url"] = build_uri("merchants_store", array("urid" => $goods["user_id"]), $goods["rz_shopName"]);
...
{
	$now = gmtime();
	$sql = "SELECT act_id FROM " . $GLOBALS["ecs"]->table("goods_activity") . " WHERE  start_time < '$now' AND end_time > '$now' AND act_type = " . GAT_SNATCH . " ORDER BY end_time ASC LIMIT 1";
	return $GLOBALS["db"]->GetOne($sql);
}
...
function get_exchange_recommend_goods($type = "", $warehouse_id, $area_id)
{
	...
	$leftJoin .= " left join " . $GLOBALS["ecs"]->table("warehouse_area_goods") . " as wag on g.goods_id = wag.goods_id and wag.region_id = '$area_id' ";
	$now = gmtime();
	$sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '{$_SESSION["discount"]}') AS shop_price, IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price, g.promote_start_date, g.promote_end_date,ga.act_name, ga.act_id, ga.ext_info, ga.start_time, ga.start_time, ga.end_time FROM " . $GLOBALS["ecs"]->table("goods_activity") . " AS ga LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON g.goods_id = ga.goods_id LEFT JOIN " . $GLOBALS["ecs"]->table("brand") . " AS b ON b.brand_id = g.brand_id LEFT JOIN " . $GLOBALS["ecs"]->table("member_price") . " AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '{$_SESSION["user_rank"]}' " . $leftJoin . "WHERE ga.act_type = '" . GAT_SNATCH . "' AND ga.start_time <= '$now' AND ga.end_time >= '$now' AND ga.is_finished < 2 ";
	$num = 11;
...
if ($_REQUEST["act"] == "bid") {
	...
	log_account_change($_SESSION["user_id"], 0, 0, 0, -$row["cost_points"], sprintf($_LANG["snatch_log"], $row["snatch_name"]));
