...
function get_new_articles($cat_id = 0, $limit = 12)
{
	...
	$sql = "SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.description, a.file_url, a.open_type, ac.cat_id, ac.cat_name  FROM " . $GLOBALS["ecs"]->table("article") . " AS a, " . $GLOBALS["ecs"]->table("article_cat") . " AS ac WHERE a.is_open = 1 AND article_type=0 and a.cat_id = ac.cat_id AND ac." . $condition . " ORDER BY a.article_type DESC, a.add_time DESC LIMIT " . $limit;
	$res = $GLOBALS["db"]->getAll($sql);
	$arr = array();
	...
	foreach ($res as $idx => $row ) {
		...
		$arr[$idx]["add_time"] = local_date("m-d", $row["add_time"]);
