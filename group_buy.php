...
function group_buy_count($keywords)
{
	...
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS["ecs"]->table("goods_activity") . " AS ga LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON ga.goods_id = g.goods_id WHERE ga.act_type = '" . GAT_GROUP_BUY . "' AND ga.start_time <= '$now' AND ga.is_finished < 3 " . $where;
...
function group_buy_list($size, $page, $keywords, $sort, $order)
{
	...
	if ($sort == "comments_number") {
		$sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, g.market_price,b.start_time AS start_date, b.end_time AS end_date FROM " . $GLOBALS["ecs"]->table("goods_activity") . " AS b LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON b.goods_id = g.goods_id WHERE b.act_type = '" . GAT_GROUP_BUY . "' $where AND b.start_time <= '$now' AND b.is_finished < 3 ORDER BY g." . $sort . " " . $order;
	}
	else {
		$sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, g.market_price,b.start_time AS start_date, b.end_time AS end_date FROM " . $GLOBALS["ecs"]->table("goods_activity") . " AS b LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON b.goods_id = g.goods_id WHERE b.act_type = '" . GAT_GROUP_BUY . "' $where AND b.start_time <= '$now' AND b.is_finished < 3 ORDER BY b." . $sort . " " . $order;
	}
...
function get_linked_goods($goods_id, $warehouse_id = 0, $area_id = 0)
{
	...
	$sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '{$_SESSION["discount"]}') AS shop_price, g.market_price, IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price,  g.promote_start_date, g.promote_end_date FROM " . $GLOBALS["ecs"]->table("link_goods") . " lg LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " AS g ON g.goods_id = lg.link_goods_id LEFT JOIN " . $GLOBALS["ecs"]->table("member_price") . " AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '{$_SESSION["user_rank"]}' " . $leftJoin . "WHERE lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 LIMIT " . $GLOBALS["_CFG"]["related_goods_number"];
...
{
	$sql = "SELECT ga.*, g.sales_volume, g.goods_thumb, g.goods_id FROM " . $GLOBALS["ecs"]->table("goods_activity") . " ga LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " g ON ga.goods_id = g.goods_id  WHERE ga.user_id = '$user_id' AND g.goods_id > 0 AND act_type = '" . GAT_GROUP_BUY . "' ORDER BY g." . $order . " LIMIT 5 ";
...
function get_merchant_group_goods($group_buy_id)
{
	...
	$sql = "SELECT ga.act_id, ga.ext_info, ga.act_name, g.goods_thumb, g.sales_volume FROM " . $GLOBALS["ecs"]->table("goods_activity") . " ga LEFT JOIN " . $GLOBALS["ecs"]->table("goods") . " g ON ga.goods_id = g.goods_id  WHERE ga.user_id = '$ru_id' AND act_type = '" . GAT_GROUP_BUY . "' LIMIT 4 ";
...
if ($_REQUEST["act"] == "list") {
	...
	if (0 < $count) {
		...
		$cache_id = $_CFG["lang"] . "-" . $size . "-" . $page . "-" . $sort . "-" . $order . "-" . $price_min . "-" . $price_max . "-" . $keywords;
	...
	if (!$smarty->is_cached("group_buy_list.dwt", $cache_id)) {
		if (0 < $count) {
			$gb_list = group_buy_list($size, $page, $keywords, $sort, $order);
